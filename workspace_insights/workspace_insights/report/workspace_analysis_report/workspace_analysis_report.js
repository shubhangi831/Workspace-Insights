...
// 					$(this).css({ 'background': '#f5f3ff', 'border-bottom': '2px solid #c4b5fd' });
// 					$(this).find('.dt-cell__content').css({ 'color': '#5b21b6', 'font-weight': '700' });
// 				} else if (label.includes('Prev Month')) {
// 					$(this).css({ 'background': '#fff7ed', 'border-bottom': '2px solid #fed7aa' });
// 					$(this).find('.dt-cell__content').css({ 'color': '#c2410c', 'font-weight': '700' });
// 				}
// 			});
...
// 			// ── Missing Sale Alert — highlight rows where purchase > 0 but sale = 0 ──
// 			const report_data = frappe.query_report.data || [];
// 			report_data.forEach((row, idx) => {
// 				if (row.month === 'TOTAL') return;
// 				const has_purchase = parseFloat(row.mt_p || 0) > 0;
// 				const has_sale     = parseFloat(row.mt_s || 0) > 0;
// 				if (has_purchase && !has_sale) {
// 					$dt.find(`.dt-row[data-row-index="${idx}"]`).find('.dt-cell').css({
// 						'background':   '#fef3c7',
...
// 			// Refresh cards + inject chart after data renders
// 			wa_refresh_cards(frappe.query_report);
...
// 		if (fn === 'balance_delta' && value !== null && value !== undefined && data.month !== 'TOTAL') {
// 			if (num === 0) return `<span style="color:var(--text-muted,#9ca3af);font-family:monospace;">—</span>`;
// 			const abs = '₹' + Math.abs(num).toLocaleString('en-IN', {minimumFractionDigits:2,maximumFractionDigits:2});
// 			return num > 0
// 				? `<span style="color:#15803d;font-family:monospace;font-weight:600;">+${abs}</span>`
// 				: `<span style="color:#dc2626;font-family:monospace;font-weight:600;">-${abs}</span>`;
// 		}
...
// 					            border-bottom:1px solid var(--border-color,#e5e7eb);background:var(--bg-color,#f9f9f9);">
// 						<button class="wa-select-all btn btn-xs btn-default" style="flex:1;font-size:11px;">Select All</button>
// 					</div>
// 					<div class="wa-options-list"></div>
// 					<div class="wa-dropdown-footer" style="padding:6px 10px;border-top:1px solid var(--border-color,#e5e7eb);
// 					            background:var(--bg-color,#f9f9f9);">
// 						<button class="wa-clear-selection btn btn-xs btn-default" style="width:100%;font-size:11px;">
// 							Clear Selection
// 						</button>
// 					</div>
// 				</div>
...
// 		const $search   = $wrap.find('.wa-search-input');
// 		const $list     = $wrap.find('.wa-options-list');
// 		let   selected  = [];   // array of selected domain strings
...
// 				$display.html(`<span style="font-weight:500;">All domains (${selected.length})</span>`);
// 			} else {
// 				$display.html(`<span style="font-weight:500;">${selected.length} domains selected</span>`);
// 			}
// 		}

// 		function applyFilter() {
// 			filter.set_value(selected.join(','));
// 			setTimeout(() => {
// 				report.refresh();
// 				wa_refresh_cards(report);
// 			}, 120);
...
// 			$list.html(filtered.map(o => `
// 				<div class="wa-option wa-cb-option" data-val="${o}"
// 				     style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;">
// 					<input type="checkbox" class="wa-cb" ${selected.includes(o) ? 'checked' : ''}
...
// 			$list.find('.wa-cb-option').on('click', function (e) {
// 				e.stopPropagation();
...
// 	const $cards = $(`
// 		<div id="wa-cards-wrap" style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:0 0 16px;">
...
// 	// Always inject cards BEFORE the filter bar — original position
// 	const $pageForm = $('.page-form');
// 	if ($pageForm.length) {
// 		$pageForm.before($cards);
// 	} else {
// 		$('.layout-main-section, .main-section').first().prepend($cards);
// 	}
...
// 	$('.page-form').append($btn).append($export_btn).append($compare_btn);
...
// 				<th style="padding:8px 12px;text-align:left;color:${color};font-size:11px;font-weight:700;">Invoice No</th>
// 				<th style="padding:8px 12px;text-align:left;color:${color};font-size:11px;font-weight:700;">Subscription</th>
// 				<th style="padding:8px 12px;text-align:right;color:${color};font-size:11px;font-weight:700;">Amount</th>
// 				<th style="padding:8px 12px;text-align:right;color:${color};font-size:11px;font-weight:700;">Qty</th>
// 				<th style="padding:8px 12px;text-align:center;color:${color};font-size:11px;font-weight:700;width:80px;"></th>
// 			</tr>
// 		</thead>
// 		<tbody>
// 			${invoices.map((inv, i) => {
...
// 					<td style="padding:7px 12px;border-bottom:1px solid var(--border-color,#f1f5f9);">${inv.invoice_number}</td>
// 					<td style="padding:7px 12px;border-bottom:1px solid var(--border-color,#f1f5f9);">${inv.subscription}</td>
// 					<td style="padding:7px 12px;border-bottom:1px solid var(--border-color,#f1f5f9);text-align:right;font-family:monospace;color:${color};">${wa_fmt(inv.amount)}</td>
// 					<td style="padding:7px 12px;border-bottom:1px solid var(--border-color,#f1f5f9);text-align:right;">${inv.quantity}</td>
// 					<td style="padding:7px 12px;border-bottom:1px solid var(--border-color,#f1f5f9);text-align:center;">
// 						<button class="btn btn-xs btn-default wa-entries-btn" data-target="${row_id}"
// 						        style="font-size:10px;">Entries</button>
// 					</td>
// 				</tr>
// 				<tr id="${row_id}" style="display:none;background:${light_bg};">
// 					<td colspan="5" style="padding:8px 16px;border-bottom:1px solid var(--border-color,#e2e8f0);">
// 						${wa_child_table_html(inv.child_rows)}
// 					</td>
// 				</tr>`;
...
// 						<div style="font-size:10px;font-weight:600;color:#1e40af;letter-spacing:.5px;margin-bottom:4px;">PURCHASE</div>
// 						<div style="font-size:16px;font-weight:700;color:#1e40af;font-family:monospace;">${wa_fmt(totalP)}</div>
// 					</div>
// 					<div style="border:1px solid #bbf7d0;border-left:3px solid #22c55e;border-radius:6px;
// 					            padding:10px 14px;background:#f0fdf4;">
// 						<div style="font-size:10px;font-weight:600;color:#15803d;letter-spacing:.5px;margin-bottom:4px;">SALE</div>
// 						<div style="font-size:16px;font-weight:700;color:#15803d;font-family:monospace;">${wa_fmt(totalS)}</div>
// 					</div>
// 					<div style="border:1px solid #e9d5ff;border-left:3px solid #a855f7;border-radius:6px;
...
// 					</td>
// 				</tr>
// 				<tr id="${inv.row_id}" style="display:none;background:#f0f4ff;">
// 					<td colspan="6" style="padding:8px 16px;border-bottom:1px solid var(--border-color,#e2e8f0);">
// 						${wa_entries_table(inv.child_rows)}
// 					</td>
// 				</tr>
...
// 					Domain: <strong>${domain}</strong>
// 				</span>
// 				<span style="font-size:14px;font-weight:700;color:${color};font-family:monospace;">
// 					Total: ${wa_fmt(grand_total)}
// 				</span>
// 			</div>
...
// 				<thead>
// 					<tr style="background:${color};">
// 						<th style="padding:8px 12px;text-align:left;color:#fff;font-size:11px;">Month</th>
// 						<th style="padding:8px 12px;text-align:left;color:#fff;font-size:11px;">Invoice No</th>
// 						<th style="padding:8px 12px;text-align:left;color:#fff;font-size:11px;">Subscription</th>
// 						<th style="padding:8px 12px;text-align:right;color:#fff;font-size:11px;">Amount</th>
// 						<th style="padding:8px 12px;text-align:right;color:#fff;font-size:11px;">Qty</th>
// 						<th style="padding:8px 12px;text-align:center;color:#fff;font-size:11px;width:70px;"></th>
// 					</tr>
// 				</thead>
// 				<tbody>${rows_html}</tbody>
...
// 			const html = `
// 			<div style="display:flex;justify-content:flex-end;margin-bottom:8px;">
// 				<input type="text" id="wa-margin-search" class="form-control"
// 				       placeholder="Search domain..."
// 				       style="width:200px;height:28px;font-size:12px;padding:4px 8px;border-radius:4px;">
// 			</div>
// 			<table id="wa-margin-tbl" style="width:100%;border-collapse:collapse;font-size:12px;">
// 				<thead>
// 					<tr style="background:#6b21a8;">
// 						<th style="padding:10px 14px;text-align:left;color:#fff;">Domain</th>
// 						<th style="padding:10px 14px;text-align:right;color:#fff;">Purchase</th>
// 						<th style="padding:10px 14px;text-align:right;color:#fff;">Sale</th>
// 						<th style="padding:10px 14px;text-align:right;color:#fff;">Margin</th>
// 					</tr>
// 				</thead>
// 				<tbody>
// 					${r.message.map((d, i) => {
...
// 	// Always require domain selection — no domain = no export
// 	if (!domains.length) {
// 		frappe.show_alert({ message: 'Please select a domain first, then export', indicator: 'orange' }, 4);
// 		return;
// 	}

// 	// Build heading based on selection
// 	const all_count = (frappe.query_report._all_domain_count || 999);
// 	const heading   = domains.length === 1 ? domains[0] :
// 	                  domains.length >= all_count ? 'All Domain Data' :
// 	                  domains.join(', ');

// 	if (type === 'all') {
// 		frappe.show_alert({ message: 'Generating Excel file...', indicator: 'blue' }, 3);
// 		frappe.call({
...
// 			args:   { domain: domain_raw, from_month, to_month, heading },
// 			callback: function (r) {
// 				if (!r.message) return;
...
// 				a.click();
// 				frappe.show_alert({ message: 'Excel downloaded!', indicator: 'green' }, 3);
// 			}
// 		});
// 		return;
// 	}
...
// 	frappe.call({
// 		method: 'workspace_insights.workspace_insights.report.workspace_analysis_report.workspace_analysis_report.get_export_detail',
// 		args:   { domain: domain_raw, from_month, to_month },
// 		callback: function (r) {
// 			if (!r.message) return;
// 			wa_download_single_csv(type, r.message, heading, from_month, to_month);
// 		}
// 	});
// });

// function wa_download_single_csv(type, detail, domain, from_month, to_month) {
...
// 	function trigger_download(filename, csv_content) {
// 		// Add domain info at the top of every CSV
// 		const info    = `"Domain:","${domain}","Period:","${from_month} to ${to_month}"\n\n`;
// 		const blob    = new Blob(['\ufeff' + info + csv_content], { type: 'text/csv;charset=utf-8;' });
...
// 	// Domain in filename e.g. "astralcad.com_Summary_Jan2025_Mar2026.csv"
// 	const suffix = `${domain}_${from_month}_${to_month}`;

// 	if (type === 'summary') {
// 		const cols    = frappe.query_report.columns || [];
// 		const rows    = frappe.query_report.data    || [];
// 		const headers = cols.map(c => c.label);
// 		const mapped  = rows.map(row => Object.fromEntries(cols.map(c => [c.label, row[c.fieldname] ?? ''])));
// 		trigger_download(`${suffix}_Summary.csv`, to_csv(headers, mapped));
// 	}

// 	const detail_headers = ['Invoice Number', 'Invoice Date', 'Domain', 'Subscription',
// 	                        'Description', 'Start Date', 'End Date', 'Quantity', 'Amount',
// 	                        'Order Name', 'PO Number'];

// 	if (type === 'purchase') {
// 		const rows = (detail.purchase || []).map(r => ({
// 			'Invoice Number': r.invoice_number, 'Invoice Date': r.invoice_date,
// 			'Domain': r.domain, 'Subscription': r.subscription,
// 			'Description': r.description, 'Start Date': r.start_date,
// 			'End Date': r.end_date, 'Quantity': r.quantity, 'Amount': r.amount,
// 			'Order Name': r.order_name, 'PO Number': r.po_number
// 		}));
// 		trigger_download(`${suffix}_Purchase.csv`, to_csv(detail_headers, rows));
// 	}

// 	if (type === 'sale') {
// 		const rows = (detail.sale || []).map(r => ({
// 			'Invoice Number': r.invoice_number, 'Invoice Date': r.invoice_date,
// 			'Domain': r.domain, 'Subscription': r.subscription,
// 			'Description': r.description, 'Start Date': r.start_date,
// 			'End Date': r.end_date, 'Quantity': r.quantity, 'Amount': r.amount,
// 			'Order Name': r.order_name, 'PO Number': r.po_number
// 		}));
// 		trigger_download(`${suffix}_Sale.csv`, to_csv(detail_headers, rows));
// 	}

// 	frappe.show_alert({ message: 'CSV downloaded!', indicator: 'green' }, 3);
// }


// // ═══════════════════════════════════════════════════════════════════
// // Compare Domains button handler
// // ═══════════════════════════════════════════════════════════════════
...
// 	// Show domain picker
// 	const d = new frappe.ui.Dialog({
// 		title: 'Compare with Domain',
// 		fields: [{
// 			fieldname: 'domain2',
// 			fieldtype: 'Data',
// 			label:     'Second Domain',
// 			reqd:      1,
// 			description: 'Enter the domain to compare against ' + domain1
// 		}],
// 		primary_action_label: 'Compare',
// 		primary_action: function (values) {
// 			d.hide();
// 			wa_open_comparison_popup(domain1, values.domain2, from_month, to_month);
// 		}
// 	});
// 	d.show();
// });
...
// 						<th colspan="3" style="padding:8px 12px;text-align:center;color:#fff;">${domain2}</th>
// 					</tr>
// 					<tr style="background:#eff6ff;">
// 						<th style="padding:6px 12px;text-align:left;color:#1e40af;"></th>
...
// 						<th style="padding:6px 12px;text-align:left;color:#1e40af;font-size:11px;">Purchase</th>
// 						<th style="padding:6px 12px;text-align:left;color:#15803d;font-size:11px;">Sale</th>
// 						<th style="padding:6px 12px;text-align:left;color:#6b21a8;font-size:11px;">Balance</th>
// 					</tr>
// 				</thead>
// 				<tbody>${rows_html}</tbody>
// 			</table>`;
...
// 	// Remove old chart if it exists (re-render on each report run)
...
// 	// Sum all purchase and sale AMT per month
...
// 	// Inject chart AFTER cards, BEFORE filter bar
// 	const $pageForm2 = $('.page-form');
// 	if ($('#wa-cards-wrap').length) {
// 		$('#wa-cards-wrap').after($wrap);
// 	} else if ($pageForm2.length) {
// 		$pageForm2.before($wrap);
// 	}
...
// 			tooltipOptions: {
// 				formatTooltipY: v => '₹' + flt(v).toLocaleString('en-IN', { minimumFractionDigits: 2 })
// 			},
// 			axisOptions: { xIsSeries: true }
// 		});
// 	} catch(e) {
// 		// Chart library not available — remove container silently
// 		$('#wa-chart-wrap').remove();
// 	}
// }
