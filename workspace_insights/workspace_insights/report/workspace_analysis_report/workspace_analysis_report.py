...
#     })
#     columns.append({
#         "label":     _("Δ vs Prev Month"),
#         "fieldname": "balance_delta",
#         "fieldtype": "Float",
#         "width":     140,
#         "precision": 2
#     })
...
#         balance        = mt_s - mt_p
#         row["balance"] = balance
#         row["mt_p"]    = mt_p   # used by JS for missing sale alert
#         row["mt_s"]    = mt_s
#         g_balance     += balance
#         data.append(row)
...
#         if i == 0:
#             row["balance_delta"] = 0.0
#         else:
#             row["balance_delta"] = row["balance"] - data[i - 1]["balance"]
...
#         total_row[fn_base + "_lic"] = last_s_lic[sub]  # last month's seats, not sum
#     total_row["balance"]       = g_balance
#     total_row["balance_delta"] = None   # No delta for TOTAL row
#     data.append(total_row)
...
# # ─────────────────────────────────────────────────────────────────
# # Export & Comparison methods
# # ─────────────────────────────────────────────────────────────────

# @frappe.whitelist()
# def get_export_detail(domain, from_month='', to_month=''):
#     """
#     Returns data for Excel export — 3 sheets:
#     1. summary   : month-by-month balance (from execute)
#     2. purchase  : all Purchase Invoice Items rows for domain+range
#     3. sale      : all Sale Invoice Items rows for domain+range
#     """
#     domain   = (domain or '').strip().lower()
#     from_key = month_key(from_month)
#     to_key   = month_key(to_month)
...
#     """
#     Returns month-by-month Purchase / Sale / Balance for two domains
#     so JS can show them side by side.
#     """
#     from_key = month_key(from_month)
#     to_key   = month_key(to_month)
...
# @frappe.whitelist()
# def generate_excel_export(domain, from_month='', to_month=''):
#     """
#     Generate a proper .xlsx file with 3 sheets:
#     Sheet 1 — Summary (month-by-month balance from execute)
#     Sheet 2 — Purchase Details (all invoice item rows)
#     Sheet 3 — Sale Details (all invoice item rows)
#     Returns base64-encoded content for JS to trigger download.
#     """
#     try:
#         import openpyxl
...
#     p_fill         = PatternFill("solid", fgColor="1E40AF")   # blue
#     s_fill         = PatternFill("solid", fgColor="15803D")   # green
#     sum_fill       = PatternFill("solid", fgColor="374151")   # dark grey
...
#     def style_data_row(ws, row_idx, is_total=False):
#         for cell in ws[row_idx]:
#             cell.border    = thin_border
#             cell.alignment = right_align if isinstance(cell.value, (int, float)) else Alignment(vertical="center")
#             if is_total:
...
#     filters = {'domain': domain, 'from_month': from_month, 'to_month': to_month}
#     if domain and domain != 'all':
#         try:
#             columns, data = execute(filters)
#             s1_headers = [c['label'] for c in columns]
#             ws1.append(s1_headers)
#             style_header_row(ws1, 1, sum_fill)

#             for row in data:
#                 ws1.append([row.get(c['fieldname'], '') for c in columns])
#                 is_total = row.get('month') == 'TOTAL'
#                 style_data_row(ws1, ws1.max_row, is_total)
...
#         except Exception:
#             ws1.append(["Summary not available — select a specific domain"])

#     # ── Sheet 2: Purchase Details ─────────────────────────────────
#     ws2 = wb.create_sheet("Purchase Details")
#     detail = get_export_detail(domain if domain != 'all' else '', from_month, to_month)

#     p_headers = ['Invoice Number', 'Invoice Date', 'Domain', 'Subscription',
#                  'Description', 'Start Date', 'End Date', 'Quantity', 'Amount',
#                  'Order Name', 'PO Number']
#     ws2.append(p_headers)
#     style_header_row(ws2, 1, p_fill)

#     for r in (detail.get('purchase') or []):
#         ws2.append([r.get('invoice_number'), r.get('invoice_date'), r.get('domain'),
#                     r.get('subscription'), r.get('description'), r.get('start_date'),
#                     r.get('end_date'), r.get('quantity'), r.get('amount'),
#                     r.get('order_name'), r.get('po_number')])
#         style_data_row(ws2, ws2.max_row)

#     for col in ws2.columns:
#         max_len = max((len(str(c.value or '')) for c in col), default=10)
#         ws2.column_dimensions[col[0].column_letter].width = min(max_len + 4, 35)

#     # ── Sheet 3: Sale Details ──────────────────────────────────────
#     ws3 = wb.create_sheet("Sale Details")
#     ws3.append(p_headers)
#     style_header_row(ws3, 1, s_fill)

#     for r in (detail.get('sale') or []):
#         ws3.append([r.get('invoice_number'), r.get('invoice_date'), r.get('domain'),
#                     r.get('subscription'), r.get('description'), r.get('start_date'),
#                     r.get('end_date'), r.get('quantity'), r.get('amount'),
#                     r.get('order_name'), r.get('po_number')])
#         style_data_row(ws3, ws3.max_row)

#     for col in ws3.columns:
#         max_len = max((len(str(c.value or '')) for c in col), default=10)
#         ws3.column_dimensions[col[0].column_letter].width = min(max_len + 4, 35)
...
#     label    = domain if domain and domain != 'all' else 'All_Domains'
#     filename = f"{label}_Report_{from_month or 'All'}_{to_month or 'Months'}.xlsx"
